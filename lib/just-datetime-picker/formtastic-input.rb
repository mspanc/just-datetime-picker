      ...
      def to_html
        input_wrapping do
          ...
          builder.text_field("#{method}_date", input_html_options.merge(
            ...
            value: builder.object.send("#{method}_date"),
            maxlength: 10,
            size: 10
          )) <<
          builder.text_field("#{method}_time_hour", input_html_options.merge(
            class: "just-datetime-picker-field just-datetime-picker-time just-datetime-picker-time-hour",
            value: hour_value,
            maxlength: 2,
            size: 2
          )) <<
          ":" <<
          builder.text_field("#{method}_time_minute", input_html_options.merge(
            class: "just-datetime-picker-field just-datetime-picker-time just-datetime-picker-time-minute",
            value: minute_value,
            maxlength: 2,
            size: 2
          ))
